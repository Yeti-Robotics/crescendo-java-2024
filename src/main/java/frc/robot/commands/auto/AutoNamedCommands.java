...
public class AutoNamedCommands {
    ...
    public void registerCommands() {
        NamedCommands.registerCommand(
                "rollIn", new StartEndCommand(() -> intakeSubsystem.roll(.9), intakeSubsystem::stop));
        NamedCommands.registerCommand(
                "rollOut", new StartEndCommand(() -> intakeSubsystem.roll(-.70), intakeSubsystem::stop));
        NamedCommands.registerCommand(
                "armUp",
                new StartEndCommand(() -> armSubsystem.moveUp(.5), armSubsystem::stop)
                        .until(() -> armSubsystem.getEnc() >= 0.75)
                        .andThen(armSubsystem::setMotorsBrake));
        NamedCommands.registerCommand(
                "armDown",
                new StartEndCommand(() -> armSubsystem.moveDown(.5), armSubsystem::stop)
                        .until(() -> armSubsystem.getEnc() <= .5 && armSubsystem.getEnc() >= .44)
        ...
        NamedCommands.registerCommand(
                "shootOut",
                new StartEndCommand(() -> shooterSubsystem.spinNeo(), shooterSubsystem::stopFlywheel)
                        .alongWith(new StartEndCommand(() -> intakeSubsystem.roll(-1), intakeSubsystem::stop)));
        NamedCommands.registerCommand(
                "handoff",
        ...
        NamedCommands.registerCommand(
                "handoffMidLine",
                new StartEndCommand(() -> intakeSubsystem.roll(.275), intakeSubsystem::stop)
                        .withTimeout(0.5)
                        .andThen(
        ...
        NamedCommands.registerCommand(
                "pivotHandoff", new InstantCommand(() -> pivotSubsystem.setPivotPosition(0.5)));
    }
}
