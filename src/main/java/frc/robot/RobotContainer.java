...
public class RobotContainer {
    ...
    private void configureBindings() {
        ...
        buttonHelper.createButton(1, 0, new StartEndCommand(() -> shooterSubsystem.setVelocity(75), shooterSubsystem::stopFlywheel).alongWith(new InstantCommand(() -> pivotSubsystem.setPivotPosition(0.5))),MultiButton.RunCondition.WHILE_HELD);
        buttonHelper.createButton(5, 0, new StartEndCommand(() -> shooterSubsystem.setVelocity(125), shooterSubsystem::stopFlywheel).alongWith(new InstantCommand(() -> pivotSubsystem.setPivotPosition(0.44))),MultiButton.RunCondition.WHILE_HELD);
...
//                () -> armSubsystem.getEnc() <= .6 && armSubsystem.getEnc() >= .58).alongWith(new PivotHomeCommand(pivotSubsystem)), MultiButton.RunCondition.WHEN_PRESSED);
        ...
        buttonHelper.createButton(11,0,new StartEndCommand(() -> shooterSubsystem.setVelocity(30), shooterSubsystem::stopFlywheel),MultiButton.RunCondition.WHILE_HELD);
//        buttonHelper.createButton(12,0,new InstantCommand(() -> elevatorSubsystem.setPosition(ElevatorConstants.ElevatorPositions.TRAP)), MultiButton.RunCondition.WHEN_PRESSED);
        buttonHelper.createButton(12,0,new InstantCommand(climberSubsystem::disengageBrake),MultiButton.RunCondition.WHEN_PRESSED);
...
//        joystick.a().whileTrue(drivetraixn.applyRequest(() -> brake));
        ...
        joystick.leftTrigger().onTrue(new InstantCommand(() -> elevatorSubsystem.setPosition(ElevatorConstants.ElevatorPositions.AMP)).andThen(new StartEndCommand(() -> pivotSubsystem.moveDown(0.25), pivotSubsystem::stop).unless(() -> pivotSubsystem.getEncAngle() < 0.4).withTimeout(0.6).andThen(new InstantCommand(() -> pivotSubsystem.setPivotPosition(0.23)).unless(() -> !elevatorSubsystem.getmagSwitch()))));
