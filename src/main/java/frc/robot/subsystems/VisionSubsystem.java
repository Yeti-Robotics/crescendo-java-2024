...
public class VisionSubsystem extends SubsystemBase {
    ...
    public Command ledOff() {
        return runOnce(() -> LimelightHelpers.setLEDMode_ForceOff(LIMELIGHT_NAME));
    }

    public Command ledOn() {
        return runOnce(() -> LimelightHelpers.setLEDMode_ForceOn(LIMELIGHT_NAME));
    }
    ...
    public Command blinkLimelight() {
        ...
        for (int i = 0; i < NUM_BLINKS; i++) {
            blinkCommands.addCommands(Commands.sequence(
                    ledOn(),
                    Commands.waitSeconds(BLINK_DELAY_SECONDS),
                    ledOff(),
                    Commands.waitSeconds(BLINK_DELAY_SECONDS)
                    ));
        }
